...
import { StatisticBar } from '../statistics'
...
const CSVViewer = ({ externalCSV, onClearExternalCSV, onNeedLogin }) => {
  ...
  // 处理外部传入的 CSV（从 Dashboard 导出）
  useEffect(() => {
    if (externalCSV && externalCSV.content) {
      parseCSV(externalCSV.content, externalCSV.fileName, externalCSV.size, externalCSV.filePath)
      // 清除外部 CSV 引用
      if (onClearExternalCSV) {
        onClearExternalCSV()
      }
    }
  }, [externalCSV, parseCSV, onClearExternalCSV])

  // 从 Cursor 导出 CSV
  const handleExportFromCursor = useCallback(async (options = {}) => {
    const ipc = getIpcRenderer()
    if (!ipc) {
    ...
    try {
      setIsExporting(true)
      // 传递日期范围参数
      const result = await ipc.invoke('download-cursor-csv', options)
      ...
      if (result.success) {
        ...
        // 检查是否是登录问题
        if (result.error.includes('登录') || result.error.includes('过期') || result.error.includes('401') || result.error.includes('403')) {
          alert(`${result.error}\n\n请重新设置 Cookie`)
          // 跳转到 Cursor Dashboard 页面
          if (onNeedLogin) {
            onNeedLogin()
          }
        } else {
  ...
  return (
    ...
    >
      ...
      <main className="csv-viewer-content">
        {data.length === 0 ? (
          <EmptyState 
            onFileSelect={handleFileSelect} 
            isLoading={isLoading}
